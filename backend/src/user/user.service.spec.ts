...
describe('UserService', () => {
  ...
  describe('findByEmail', ()=>{
    ...
    it('should throw error when user not found', async ()=>{
      ...
      await expect(sut.findByEmail('some@mail.com')).rejects.toThrowError(new NotFoundException('User with given email not found'))
      expect(userRepository.findByEmail).toBeCalledTimes(1)
  ...
  describe('findById', () => {
    ...
    it('should throw error when user not found', async ()=>{
      ...
      await expect(sut.findById(404, locationAdminMock)).rejects.toThrowError(new NotFoundException('User with given ID not found'))
      expect(userRepository.findById).toBeCalledTimes(1)
      expect(userRepository.findByEmail).toBeCalledTimes(1)
    })
    ...
    it('should throw error when logged user role location differ from found user location', async ()=>{
      ...
      await expect(sut.findById(2, locationAdminMock)).rejects.toThrowError(new ForbiddenException('User with given ID is not in your location!'))
      expect(userRepository.findById).toBeCalledTimes(1)
      expect(userRepository.findByEmail).toBeCalledTimes(1)
    })
  })
  ...
  describe('create', () => {
    ...
    it('should throw error if logged user is creating not location user', async ()=>{
      await expect(sut.create({...userToCreate, role: Role.LOCATION_ADMIN}, locationAdminMock)).rejects.toThrowError(new ForbiddenException('You cannot create other admins'))
    })
    ...
    it('should override locationId value if logged user is location admin', async()=>{
      ...
      expect(userRepository.create.mock.calls[0][0].locationId).toEqual(locationAdminMock.locationId)
    })
    ...
    it('should hash password and create user if valid args are provided', async ()=>{
      ...
      expect(userRepository.create).toBeCalledTimes(1)
      expect(userRepository.create.mock.calls[0][0].password !== userToCreate.password).toBeTruthy()
  ...
  describe('update', () => {
    ...
    it('should throw error when logged user is location admin and updates other admins', async()=>{
      userRepository.findById.mockResolvedValueOnce({...users[0], role: Role.ADMIN})
      userRepository.findByEmail.mockResolvedValueOnce(locationAdminMock)

      await expect(sut.update(updatedUser, 1, locationAdminMock)).rejects.toThrow(new ForbiddenException('You cannot update other admins'))
      expect(userRepository.findById).toBeCalledTimes(1)
      expect(userRepository.findByEmail).toBeCalledTimes(1)
    })
    ...
    it('should throw error when logged user locationId differ from updated user locationId', async ()=>{
      ...
      await expect(sut.update(updatedUser, 1, locationAdminMock)).rejects.toThrow(new ForbiddenException('User with given ID is not in your location!'))
      expect(userRepository.findById).toBeCalledTimes(1)
      expect(userRepository.findByEmail).toBeCalledTimes(1)
    })
    ...
    it('should throw error when user not found', async()=>{
      ...
      await expect(sut.update(updatedUser, 1, locationAdminMock)).rejects.toThrowError(new NotFoundException('User with given ID not found'))
      expect(userRepository.findById).toBeCalledTimes(1)
      expect(userRepository.findByEmail).toBeCalledTimes(1)
    })
  ...
  describe('delete', () => {
    it('should throw error when user not found', async()=>{
      ...
      await expect(sut.delete(1, locationAdminMock)).rejects.toThrowError(new NotFoundException('User with given ID not found'))
      expect(userRepository.findById).toBeCalledTimes(1)
      expect(userRepository.findByEmail).toBeCalledTimes(1)
    })
    ...
    it('should throw error when logged user is trying to delete other admin', async ()=>{
      ...
      await expect(sut.delete(1, locationAdminMock)).rejects.toThrow(new ForbiddenException('You cannot delete other admins'))
      expect(userRepository.findById).toBeCalledTimes(1)
      expect(userRepository.findByEmail).toBeCalledTimes(1)
    })
